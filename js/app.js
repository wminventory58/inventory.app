...
// --- Tag Requirement Toggle Functions ---

/**
 * Saves the tag requirement toggle state to local storage.
 */
function saveTagRequirementState() {
  const isRequired = tagRequiredToggle.checked;
  localStorage.setItem("tagRequired", JSON.stringify(isRequired));
}

/**
 * Loads the tag requirement toggle state from local storage.
 */
function loadTagRequirementState() {
  const savedState = localStorage.getItem("tagRequired");
  if (savedState !== null) {
    tagRequiredToggle.checked = JSON.parse(savedState);
  } else {
    // Default to required (checked) if not set
    tagRequiredToggle.checked = true;
  }
}

/**
 * Checks if Tag # is currently required based on toggle state.
 * @returns {boolean} - True if Tag # is required, false otherwise.
 */
function isTagRequired() {
  return tagRequiredToggle.checked;
}

// --- End Tag Requirement Toggle Functions ---
...
const tagRequiredToggle = document.getElementById("tag-required-toggle");
...
function renderTable(data) {
  ...
  data.forEach((item) => {
    ...
    row.innerHTML = `
      <td class="tag-uom-qty-col"><input type="text" class="tag-input" data-id="${itemIndex}" value="${item["Tag #"] || ""}"></td>
      <td>${item["Part #"] || ""}</td>
      <td>${item.Description || ""}</td>
      <td>${item.Category || ""}</td>
      <td class="location-notes-col">
        <select class="location-select" data-id="${itemIndex}">
          ${locationOptionsHtml}
        </select>
      </td>
      <td class="tag-uom-qty-col">
        <select class="uom-select" data-id="${itemIndex}">
          ${uomOptionsHtml}
        </select>
      </td>
      <td class="tag-uom-qty-col"><input type="number" step="any" class="quantity-input" data-id="${itemIndex}" value="${item.Quantity || ""}"></td>
      <td class="location-notes-col"><input type="text" class="notes-input" data-id="${itemIndex}" value="${item.Notes || ""}"></td>
     <td><button class="btn btn-primary btn-sm submit-count-btn" data-id="${itemIndex}">Submit</button></td>
    `;
...
itemListBody.addEventListener("click", (event) => {
  ...
  if (target.classList.contains("submit-count-btn")) {
    const itemIndex = parseInt(target.dataset.id, 10);
    ...
    // Check Tag # only if required
    if (isTagRequired() && (!tagInput.value || !tagInput.value.trim())) {
    ...
    if (!isValid) {
      // Dynamic error message based on toggle state
      const requiredFieldsMessage = isTagRequired()
        ? "Please fill out all required fields (Tag #, Location, UOM, and QTY)."
        : "Please fill out all required fields (Location, UOM, and QTY).";
      showNotification(requiredFieldsMessage);
...
// Event listener for the tag requirement toggle
tagRequiredToggle.addEventListener("change", saveTagRequirementState);
...
addItemForm.addEventListener("submit", (event) => {
  ...
  // Build required fields array based on toggle state
  ...
  // Add tagNo to required fields only if Tag # is required
  if (isTagRequired()) {
    requiredFields.unshift("tagNo");
  }

  let isFormValid = true;

  // Clear previous validation states from all possible required fields
  ["tagNo", "description", "modal-location", "modal-uom", "quantity"].forEach(
    (id) => {
  ...
  delete displayItem["Tag #"];
  delete displayItem.Notes;
...
// --- Real-time Validation for Modal ---
...
fieldsToValidate.forEach((fieldId) => {
  ...
  if (input) {
    ...
    input.addEventListener(eventType, () => {
      // For tagNo, only validate if Tag # is currently required
      if (fieldId === "tagNo" && !isTagRequired()) {
        return; // Skip validation for tagNo if not required
      }
      if (input.value.trim() !== "") {
        input.classList.remove("is-required");
